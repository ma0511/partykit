...
In the following analysis of the \textit{Bookbinder's Book Club} data \code{evtree} is used in conjunction with \code{rpart} and \code{ctree}. All trees are constrained to have a minimum number of 26 observations per terminal node ($2\%$ of the total number of observations). To avoid excessively large trees \code{ctree} is called with $\alpha = 1\%$. To provide uniform visualizations and predictions of the fitted models the \code{as.party()} function is used to convert \pkg{rpart} objects into objects of class \pkg{party}. The sample session starts with the construction and visualization of trees using the \code{rpart} and \code{ctree} methods. 
%
<<recursive-trees>>=
data("BBBClub", package = "evtree")
library("rpart")
set.seed(1090)
rp <- as.party(rpart(choice ~ ., data = BBBClub, minbucket = 26))
ct <- ctree(choice ~ ., data = BBBClub, mincrit = 0.99, minbucket = 26)
plot(rp)
plot(ct)
@
%
...
@
\end{minipage}
\caption{\label{fig:example_rpart_ctree} Trees constructed by \code{rpart} (upper panel) and \code{ctree} (lower panel). Target variable is the customers choice of buying the book. The variables used for splitting are: \code{art} number of art books purchased; \code{first} number of months since the first purchase; \code{freq} number of books purchased at the BBB Club; \code{gender} the costumers gender.
}
\end{figure}
%
...
%
<<evtree-plot>>=
ev <- evtree(choice ~ ., data = BBBClub, minbucket = 26, maxdepth = 2)
plot(ev)
print(ev)
@
%
...
plot(ev)
@
\caption{\label{fig:example_evtree} Tree constructed by \code{evtree}. Target variable is the customers choice of buying the book. The variables used for splitting are: \code{art} number of art books purchased; \code{first} number of months since the first purchase}
\end{figure}
%
...
@
The accuracy of the three models are similar. With a misclassification rate of $24.3\%$, the \code{evtree} model is slightly worse than the \code{rpart} model ($23.8\%$) and better than the \code{ctree} model ($24.9\%$). 
%
<<evtree-structure>>=
...
@
%
\code{rpart} and \code{ctree} models are fitted with the same constrained on the tree's depth (\code{maxdepth = 2}) as the previous \code{evtree} model. With a misclassification rate of $24.3\%$, the \code{evtree} model is more accurate than the \code{rpart} model ($26.2\%$) and the \code{ctree} model ($25.5\%$). This shows an particular advantage of \code{evtree}, which is its ability to built the best tree for a given tree size. 
