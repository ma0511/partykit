...
recursively for and subsequently \emph{pruned} to satisfy the complexity tradeoff:
%
\begin{eqnarray} \label{eq:locally}
  ...
  \bar \theta
    & = & \argmin_{\bar \theta_r} \; \mathrm{loss}\{Y, f(X, \bar \theta_r)\} ~+~ \mathrm{comp}(\bar \theta_r).
  \label{eq:pruning}
\end{eqnarray}
%
...
Through a global search up to $M-1$ levels (\ref{eq:globally}) the complexity
increases to $O(P^M N^M)$ \citep{papagelis2001bdt}. One conceivable compromise
between these two extremes is to look ahead $d$~steps with $1 < d < M$
...
can be found, then $\tilde{\theta}_{i+1} = \tilde{\theta}_{i}$. Otherwise, the
set of parameters in iteration $i+1$ are given by $\tilde{\theta}_{i+1}=
\tilde{\theta}_i \cup (v_{r},s_{r})$.
...
parameters at iteration $i+1$ are $\tilde{\theta}_{i+1}=\tilde{\theta}_{i}
\setminus (v_{r},s_{r})$. If $\tilde{\theta}_{i}$ only comprises one internal
node, i.e., the root node, then $\tilde{\theta}_{i+1} = \tilde{\theta}_{i}$.
...
If no pruning occurs, $\tilde{\theta}_{i}$ and $\tilde{\theta}_{i+1}$ contain
...
$\tilde{\theta}_i$. Thus, $\tilde{\theta}_{i+1} = \tilde{\theta}_{i} \setminus
...
solution $\tilde{\theta}_{i+1} = \tilde{\theta}_{i} \setminus (v_{m_1}, s_{m_1},
..., v_{m_f}, s_{m_f})$.

\subsubsection{Crossover}

\fixme{I really had problems following this section. The sub-operator is useful
but is it possible to introduce this with fewer sub- and super-scripts? It makes
it very challenging to read.}

\textit{Crossover} exchanges, randomly selected, subtrees between two trees. Let
$\tilde{\theta}_{i}^1$ and $\tilde{\theta}_{i}^2$ be the parameters of the trees
that are chosen for crossover. First, an internal node is selected randomly from
each of $\tilde{\theta}_{i}^1$ and $\tilde{\theta}_{i}^2$, which we denote as
$n_k^1$ and $n_k^2$. Let $\mathrm{sub}1(\tilde{\theta}_i^j, n^j_k)$ denote the subtrees
rooted by $n_k^j$ ($j=1,2$). Then, $\mathrm{sub}1(\tilde{\theta}_i^j) =
(v_{n^j_k}, s_{n^j_k},  v_{n^j_{k1}}, s_{n^j_{k1}}, \dots ,v_{n^j_{km}}, s_{n^j_{km}})$, where $(n^j_{k1}, ..., n^j_{km})$ are the set
of descendent node positions of $n^j_k$. Now, the two selected subtrees can be
written as $\mathrm{sub}1(\tilde{\theta}_{i}^1, n^1_k) $ and $\mathrm{sub}1(\tilde{\theta}_{i}^2,
n^2_k)$. Furthermore, $\mathrm{sub}2(\tilde{\theta}_{i}^1, n^1_k) = \tilde{\theta}_{i}^1
\setminus \mathrm{sub}1(\tilde{\theta}_{i}^1, n^1_k)$ and $\mathrm{sub}2(\tilde{\theta}_{i}^2,
n^2_k) = \tilde{\theta}_{i}^2 \setminus \mathrm{sub}1(\tilde{\theta}_{i}^2, n^2_k)$. 

After the application of the \textit{crossover} operator, the trees' parameter
spaces are given by $\tilde{\theta}_{i+1}^1= \mathrm{sub}2(\tilde{\theta}_{i}^1, n^1_k)
\cup \mathrm{sub}1(\tilde{\theta}_{i}^2, n^2_k) \setminus (v_{m^2_{1}}, s_{m^2_{1}}, ...,
v_{m^2_{f}}, s_{m^2_{f}})$ and $\tilde{\theta}_{i+1}^2=
\mathrm{sub}2(\tilde{\theta}_{i}^2, n^2_k) \cup \mathrm{sub}1(\tilde{\theta}_{i}^1, n^1_k)
\setminus (v_{m^1_{1}}, s_{m^1_{1}}, ..., v_{m^1_{f}}, s_{m^1_{f}})$. Here,
$(v_{m^1_{1}}, s_{m^1_{1}}, ..., v_{m^1_{f}}, s_{m^1_{f}}) \subset
\mathrm{sub}1(\tilde{\theta}_i^1, n^1_k)$ and $(v_{m^2_{1}}, s_{m^2_{1}}, ...,
v_{m^2_{f}}, s_{m^2_{f}}) \subset \mathrm{sub}1(\tilde{\theta}_i^2, n^2_k)$ are the
parameters of the invalid nodes.
...
mutation operator either the solution before $\tilde{\theta}_i$ or after
modification $\tilde{\theta}_{i+1}$ is kept in memory. In the case of the
crossover operator, the initial solutions of $\tilde{\theta}_i^1$ competes with
its subsequent solutions $\tilde{\theta}_{i+1}^1$. Correspondingly, one of the
two solutions $\tilde{\theta}_i^2$ and $\tilde{\theta}_{i+1}^2$ is rejected. The
...
\end{figure}
%
The resulting evolutionary tree is printed output below and visualized in
...
\citep{LKF2009}. The analysis is based on the evaluation of 250~bootstrap
...
the Austrian DRG-system are described. 
\fixme{The DRG description is far too long. All readers are waiting for the results
of the benchmark now. One explanatory sentence -- or maybe two at most -- should be
used to state the general problem and point to the manual pages.}
In the Austrian DRG-system, resources are
allocated to hospitals by simple rules regarding diagnosis, selected individual
services, and patient characteristics. Within this system regression tree
analysis is performed to classify hospital patients into clinically meaningful
and comprehensible groups that consume similar hospital resources. The datasets
MEL0101 (intervention on the skull), HDG0202 (malignant neoplasms), and HDG0502
(acute affections of the respiratory tract and middle atelectasis) contain
patient groups with the same main diagnosis or procedure. The task of the
regression tree analysis is to further divide these groups with the intention of
finding groups with similar resource consumption. Possible split-variables are
the patients' diagnoses, procedures, age, and gender. The dependent variable is
the patients' length of stay, which has a good relationship with the total costs
and is available \citep{theurl2007ihf}. Permission to use the data was granted
by the \citet{BMG2009}. A more detailed description of the application can be
found in \citet{grubinger2010regression}.
...
The disadvantages of the \code{evtree} algorithm are computation time and memory requirements. While the smallest of the analyzed datasets, \textit{Glass identification},
...
\fixme{also briefly discuss the issue of instability of the evolutionary trees even for large populations and number of runs}
...
classification and regression trees that are grown by through an evolutionary
