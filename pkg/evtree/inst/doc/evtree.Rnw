...
%
\begin{equation}
	\hat{\theta} = \argmin_{\theta \in \Theta} \; \mathit{loss}(Y, f(X,\theta)) + \mathit{comp}(\theta).
...
%
\begin{equation}
		\min_{v_r, s_r} \; \mathit{loss}(Y, f(X, \tilde{\theta}_{i-1} \cup (v_r, s_r) ))
	\label{eq:whichsplit}
\end{equation}
%
...
%
\begin{equation}
		\min_{r=1,...,n_{M-1}} \; \mathit{loss}(Y, f(X,\tilde{\theta}_i \setminus (v_r, s_r )))  + \mathit{comp}(\tilde{\theta}_i \setminus (v_r, s_r)).
	\label{eq:pruning}
\end{equation}
%
...
\begin{equation}
	 \hat{\theta} = \argmin_{\theta \in \Theta} \; 2 \cdot MC(Y, f(X, \theta))+\alpha \cdot M \cdot \ln N.
...
\begin{equation}
	 \hat{\theta} = \argmin_{\theta \in \Theta} \; N \cdot \ln \mathit{MSE}(Y, f(X, \theta))+\alpha \cdot 4 \cdot (M+1) \cdot \ln N.
\label{eq:regression}
\end{equation} 
