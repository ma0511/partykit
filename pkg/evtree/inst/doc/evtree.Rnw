...
r <- rbind(r, preprocess(d = rmagicgamma, dname = "Magic Gamma Telescope", isclassification = T))
...
r <- rbind(r, preprocess(d = rmel0101, dname = "MEL0101", isclassification = F))
r <- rbind(r, preprocess(d = rhdg0202, dname = "HDG0202", isclassification = F))
...
plotd <- function(alg){
    ...
    stats <- cstats(ra, alg)
    plotCI(stats$mean, dummy, li=stats$lower, ui=stats$upper, err="x", xlim=c(-11,46), axes=F, ann=T, ylab= "", xlab= "difference in predictive accuracy [%]", lwd=1.5 )
    ...
    stats <- cstats(rc, alg)
    plotCI(stats$mean, dummy, li= stats$lower, ui= stats$upper, err="x", xlim=c(-110,210), axes=F, ann=T, ylab= "", xlab= "difference in complexity [%]", lwd=1.5 )
