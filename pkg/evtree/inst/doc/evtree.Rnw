...
\end{equation}
%
where $r \in \{1,...,M-1\}$ are the internal \emph{nodes}, $v_r \in \{1, \dots ,P\}$ the associated splitting
...
No obvious relationship between the data characteristics and \code{evtree}'s relative
performance could be observed. Probably, one explanation is that for some datasets,
the recursive partitioning algorithms already yield (nearly) optimal trees, leaving little room for further
improvements. For other datasets, \code{evtree}'s global search strategy can find more
