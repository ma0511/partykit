...
ev <- evtree(Species ~ ., iris, control = evtree.control(seed = 1000))
@
%
...
\begin{figure}[t!]
\centering
<<iris-plot, fig=TRUE, echo=FALSE, width=13.2, height=8>>=
plot(ev)
@
...
%
<<iris-predict>>=
predict(ev, newdata = iris[c(10, 35, 60, 85, 110, 135), -5])
@
