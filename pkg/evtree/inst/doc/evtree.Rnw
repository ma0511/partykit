...
@
%
Although the constructed trees are considerably different, the structure of the predictions are relatively similar. In this case, \code{evtree} classifies less customers ($186$) as buyers as \code{rpart} ($216$) and \code{ctree} ($278$). Of the customers classified as buyers, $72.6\%$ (\code{evtree}), $70.8\%$ (\code{rpart}) and $63.7\%$ (\code{ctree}) actually bought the book. In a final step \code{rpart} and \code{ctree} models are fitted with the same constrained on the tree's depth (\code{maxdepth = 2}) as the previously fitted \code{evtree} model.
%
<<evtree-performance>>=
rp <- as.party(rpart(choice ~ ., data = BBBClub, minbucket = 26, 
maxdepth = 2))
