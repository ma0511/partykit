...
We added the following paragraph to the ``Conclusions'', as an explanation as to when and why evtree outperforms its competitors:
...
the recursive partitioning trees are already (nearly) optimal. For other datasets, 
 evtree's global search strategy can find more complex interactions, 
...
The rules are now presented in tabular format and are now easier to read.
