...
is used to call the \code{J48} implementation in \pkg{Weka}). Finally, Section~\ref{conc} gives
...
In this section, we compare \code{evtree} with \code{rpart}, \code{ctree} and \code{J48} (Classification datasets)
...
depth of~9. \code{J48} is only constrained to  a minimum number of 
7~observations per terminal node, as other restrictions are not possible via the \pkg{RWeka} interface. 
Apart from that, the default settings of the algorithms are used. For the assessment
...
The relative performance of \code{evtree} and \code{J48} is summarized in Figure~\ref{fig:benchmark2}. It can be observed that on $8$ out of $11$ classification datasets \code{evtree} significantly outperforms \code{J48} in terms of predictive accuracy. \code{evtree}'s performance is significantly worse on the \textit{Vowel} dataset, where the misclassification error of \code{evtree} is $2.7\%$ higher. \code{evtree} model are significantly less complex on all 11 datesets. -- simulation mit prunden -- 
...
\caption{\label{fig:benchmark2} Performance comparison of \code{evtree} vs.\ \code{J48} .
  Prediction error (left panel) is compared by the relative difference of the misclassification rate.  The complexity (right panel) is compared by the relative difference of the total number of nodes.}
\end{figure}
...
In this section, the choice of -- uebersicht
...
\begin{figure}[t!]
\centering
<<param-operator-plot, fig = TRUE, echo = FALSE, width=10.2, height=15.5>>=
	par(mfrow = c(4, 3), oma = c(5, 10, 2, 0), mar = c(1, 1, 2, 1))
	plot(b1)
@
\caption{\label{fig:param} Text}
\end{figure}
...
\begin{figure}[t!]
\centering
<<param-ntrees-plot, fig = TRUE, echo = FALSE, width=10.2, height=5.2>>=
	par(mfrow = c(4, 1), oma = c(5, 10, 2, 0), mar = c(1, 1, 2, 1))
	plot(b2)
@
\caption{\label{fig:param} Text2}
\end{figure}
