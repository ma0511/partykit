...
\Abstract{
  ...
  \citep{therneau1997introduction}, the open-source CART implementation, 
  conditional inference trees \citep[\code{ctree},][]{hothorn2006urp}, and \code{J48}, the 
  open-source C4.5 implementation . The usefulness of \code{evtree} is illustrated in
  a textbook customer classification task  and a benchmark study of predictive 
  accuracy in which \code{evtree} achieved at least similar and most of the time better results compared to
  the recursive algorithms \code{rpart}, \code{ctree}, and \code{J48}.  
}
...
relationships with high order interactions and different variable types. Although
decision trees are not always competitive with the best methods in terms of predictive
accuracy \citep{caruana2006empirical, hastie}, their models have the advantage
of being easily interpretable. The resulting model can be interpreted as a tree structure providing a compact
...
of tree algorithms such as \code{J48} (the open source implementation of \textit{C4.5}) 
or \code{M5P} (open source implementation of \textit{M5}) \citep{quinlan1992lcc}.
...
simulated annealing. The \pkg{GA} \cite{scrucca2013} provides a collection of general 
...
following, we use a steady state algorithm with deterministic crowding (i.e., one parent
solution competes with it's most similar offspring for a place in the population)
...
\code{ctree} (from the \pkg{party} package) and \code{J48} (the  \pkg{RWeka} interface
is used to call the \code{J48} implementation in \pkg{Weka}). In Section \ref{param},
the fine tuning of user defined parameters is investigated. Finally, Section~\ref{conc} gives
...
regression tree induction is provided by \citep{barros2012survey}. A survey on the application of genetic
programming to classification problems -- including classification trees -- can be found
in \citep{espejo2010survey}.
...
Our algorithm uses deterministic crowding, where each parent solution
...
compared to forward-search recursive partitioning. Note that, \code{J48} is not included in this section, 
because the resulting model has a tree depth of $8$ (also when pruning is turned on). Thus, an
easy interpretation of the result is impossible. The \pkg{RWeka}
interface does not allow for restriction to the depth of \code{J48} trees.
...
In the first part of the analysis (Section~\ref{comp1})  \code{evtree}, \code{rpart}, and \code{ctree} are
...
\citep{LKF2011}. As \code{J48} can only be used for classification, the algorithm is only compared 
on the 12~classification datasets. 
...
for calculating simultaneous $95\%$ confidence intervals on the individual datasets. Thus, confidence intervals
...
The relative performance of \code{evtree} and \code{J48} is summarized in Figure~\ref{fig:benchmark2}. It can be observed that on $8$ out of $11$ classification datasets \code{evtree} significantly outperforms \code{J48} in terms of predictive accuracy. \code{evtree}'s performance is significantly worse on the \textit{Vowel} dataset, where the misclassification error of \code{evtree} is $2.7\%$ higher. As \code{J48} allows multiway splits, the complexity of the two algorithms is compared by the total number of nodes (internal nodes + terminal nodes). \code{evtree} model are significantly less complex on all 11 datesets \footnote{Note that, we also simulated \code{J48} with pruning, which resulted in much smaller trees. However, the resulting trees were still much larger than the \code{evtree} trees, and the predictive performance were much worse than the unpruned simulation of \code{J48} -- with pruning \code{J48} also performed significantly worse on the \textit{Vowel}, \textit{Musk} and \textit{Spam} datasets. Therefore, we included the unpruned version into the results of this paper.}. 
...
The results of \code{evtree}, \code{rpart} and \code{ctree} are summarized in Table~\ref{tab:resultsChessboard}. It can be seen that, in the absence of noise, \code{rpart} models on average classify $69.1\%$ of the data points correctly and has $16.6$ terminal nodes. An average \code{ctree} model has only $1.1$ terminal nodes and a classification accuracy of $49.9\%$. In contrast, \code{evtree} classifies $93.2\%$ of the instances correctly and requires $14.4$ terminal nodes. In this comparison, the complexity of  \code{J48} is again measured by the total number of nodes. On average, \code{J48} trees have only $1.2$ nodes and classify $50.0\%$ of the datapoints correctly. In the presence of $5\%$ and $10\%$ noise, \code{evtree} classifies $89.0\%$ and $84.5\%$ of the data correctly. 
...
<<chessboard-table2, echo=FALSE, results=tex>>=
...
chesstable2 <- chesstable_means2
for(j in 1:ncol(chesstable_means2)){
	for(i in 1:nrow(chesstable_means2)){
		chesstable2[i,j] <- paste(chesstable_means2[i,j] ,  "(", chesstable_sd2[i,j], ")",  sep="")	
	}
}

chesstable2 <- cbind(rbind("0\\%","5\\%","10\\%"), chesstable2)
colnames(chesstable2)[1] <- ""

print(xtable(chesstable2,
caption = "Mean (and standard deviation) of accuracy and total number of nodes for \\code{J48} on the simulated $4 \\times 4$ chessboard examples.",
caption.placement= "bottom",
label= "tab:resultsChessboard2"), 
include.rownames = FALSE, allign= "rll", hline.after=NULL,
sanitize.text.function = identity,
add.to.row=list(pos=list(-1,-1, 0, 3), command=c(
"\\toprule", 
c("{Noise} & {Accuracy}  & {Total number of nodes}  \\\\",
"\\midrule",
"\\bottomrule"
)))
)
...
In this section, \code{evtree} is simulated with different parameter choices. In general, the optimal choice of parameters depends heavily on the particular dataset. This section gives some insight into which parameters choices work for which kind of data complexity. Furthermore, the results give insight into the robustness of the default parameter choices. 

As in Section~\ref{comp}, \code{evtree} models are constrained to a minimum number of 
7~observations per terminal node, 20~observations per internal node, and a maximum tree
depth of~9. The analysis is based on 4 datasets (\textit{Statlog heart}, \textit{Statlog german credit}, \textit{Spam}, and the \textit{4x4 Chessboard problem} --  with 5\%noise). The analysis of each of the 4 datasets is based on 250~bootstrap samples. In Section \ref{param_op}, \code{evtree} is simulated with a diverse set of variation operator probability choices. Section \ref{param_op}, provides results for the choice of different population sizes.


\subsection{Variation operator probabilities}
\label{param_op}

Table \ref{tab:simoperatorprob} displays the different operator probability settings that are used in Figure~\ref{fig:param_operatorprob}. The \textit{Mutation} column summarizes the probability of selecting the \textit{minor split rule mutation} or the \textit{major split rule mutation} operator -- which both have the same probability. The \textit{Split/Prune} column summarizes the probability of selecting the \textit{prune} or the \textit{split} operator -- again both operator have the same probability. For example, the operator probability setting \textit{c0m50sp50} has a $0\%$ probability of selecting the crossover operation, a $50\%$ probability for selecting one of the mutation operators ($25\%$ for \textit{minor split rule mutation} and $25\%$ for \textit{major split rule mutation}) and a $50\%$ probability for selecting the split ($25\%$ chance) or the prune operator ($25\%$ chance). 
...
\bottomrule
\end{tabular}
\caption{\label{tab:simoperatorprob} Operator probability settings used for the simulation of the different operator probabilities. The results are displayed in Figure \ref{fig:param_operatorprob}.} 
\end{table}

\subsection{Population size}
\label{param_ps}

Figure \ref{fig:param_operatorprob} summarizes \code{evtree}'s performance with different variation operator probabilities. For the two datasets \textit{Statlog heart}  and \textit{Statlog German credit}, the results of the different variation operator settings are nearly the same. The simulations with only $200$ iterations and $500$ iterations give similar results as the simulations with $10000$ iterations. Thus, for the two smaller datasets, neither the choice of variation operator probabilities, nor the duration of the search, has a relevant effect on the results. 

For the more complex \textit{spam} dataset and the \textit{Chessboard 4x4} problem, an increased number of iteration leads to a much better performance.  For the simulation with only 200 iterations, large differences between the variation operator setups can be observed. For both datasets the variation operator setting without crossover performed worst.  The performance differences of the different variation operator settings become smaller with an increasing number of iterations.  For the \textit{Spam} dataset, a search over 10000 iterations leads to approximately the same performance for all variation operator settings. In case of the \textit{Chessboard 4x4} problem the best setting ($c20m20sp60$; $20\%$ crossover, $20\%$ mutation and $60\%$ split/prune) classifies, in average, $90.8\%$ of the instances correctly. \code{evtree}'s default setting ($c20m40sp40$; $20\%$ crossover, $40\%$ mutation and $40\%$ split/prune) is the second-best setting and classifies $89.8\%$ of the instances correctly.


\setkeys{Gin}{width=1\textwidth}
\begin{figure}[tbh!]
\centering
<<param-operator-plot, fig = TRUE, echo = FALSE, width=10.2, height=14.5>>=
	par(mfrow = c(4, 3), oma = c(5, 10, 2, 0), mar = c(1, 1, 2, 1))
	plot(b1)
@
\caption{\label{fig:param_operatorprob} Results of \code{evtree} with different variation operator probabilities and number of iterations. The operator probability settings that are display on the x-achses are described in Table \ref{tab:simoperatorprob}. In addition to the standard boxplot statistics, the mean differences are indicated by red dots.}
\end{figure}

\subsection{Population size}
\label{param}

In this subsection, \textit{evtree} is simulated with population sizes between 25 trees and 500 trees. The results of the simulation are illustrated in Figure \ref{fig:param_operatorprob}. For the two smaller datasets \textit{Statlog heart} an \textit{Statlog German credit} the results are similar for all population sizes. in fact, the smallest population sizes (25 trees; \textit{Statlog heart}: $77.1\%$, \textit{Statlog German credit}: $72.2\%$) even slightly outperformed the largest population size (500 trees; \textit{Statlog heart}: $76.5\%$, \textit{Statlog German credit}: $71.7\%$). The reason is, that In these cases the increased search space resulted in slightly overfitted trees. The average number of terminal nodes for the largest population size are much larger than the trees from the smalles population size (\textit{Statlog heart}: $7.3$ terminal nodes with a population of 500 trees, $6.5$ terminal nodes with a population of 25 trees;
 \textit{Statlog German credit}:  $13.4$ terminal nodes with a population of 500 trees, $10.9$ terminal nodes with a population of 25 trees).
 
For the more complex datasets \textit{Spam} and \textit{Chessboard 4x4 (with $5\%$ noise)}, the predictive accuracy is monotonically increasing with the search space. The largest performance difference can be observer on the chessboard simulation problem. With 25 trees only $\%81.0$ percent of the instance can be classified correctly, while the simulation with 500 trees classifies $93.6\%$ of the instances correctly. 


\setkeys{Gin}{width=1\textwidth}
\begin{figure}[tb!]
\centering
<<param-ntrees-plot, fig = TRUE, echo = FALSE, width=10.2, height=5.0>>=
	par(mfrow = c(4, 1), oma = c(5, 10, 2, 0), mar = c(1, 1, 2, 1))
	plot(b2)
@
\caption{\label{fig:param_populationsize} Results of  \code{evtree} with different population sizes. In addition to the standard boxplot statistics, the mean differences are indicated by red dots.}
\end{figure}
...
In Section~\ref{param}, we compared different parameter settings for the \code{evtree} algorithm. It could be observed
