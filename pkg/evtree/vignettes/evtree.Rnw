...
# append results of J48
rchessboard44_0 <- cbind(rchessboard44_0[,c(1:3)], rchessboard44_02[,3], rchessboard44_0[,c(4:6)], rchessboard44_02[,4])
rchessboard44_5 <- cbind(rchessboard44_5[,c(1:3)], rchessboard44_52[,3], rchessboard44_5[,c(4:6)], rchessboard44_52[,4])
rchessboard44_10 <- cbind(rchessboard44_10[,c(1:3)], rchessboard44_102[,3], rchessboard44_10[,c(4:6)], rchessboard44_102[,4])
...
names(chesstable_means) <-  c("\\code{evtree}", "\\code{rpart}", "\\code{ctree}", "\\code{J48}", "\\code{evtree}", "\\code{rpart}", "\\code{ctree}", "\\code{J48}")
chesstable_means[,1:4] <-  format(chesstable_means[,1:4]*100, digits=1, nsmall=1)
chesstable_means[,5:8] <-  format(chesstable_means[,5:8], digits=1, nsmall=1)

chesstable_sd  <- as.data.frame( rbind(apply(rchessboard44_0,2,sd), apply(rchessboard44_5,2,sd) , apply(rchessboard44_10,2,sd) )) 
names(chesstable_sd) <-  c("\\code{evtree}", "\\code{rpart}", "\\code{ctree}", "\\code{J48}", "\\code{evtree}", "\\code{rpart}", "\\code{ctree}", "\\code{J48}")
chesstable_sd[,1:4] <-  format(chesstable_sd[,1:4]*100, digits=1, nsmall=1)
chesstable_sd[,5:8] <-  format(chesstable_sd[,5:8], digits=1, nsmall=1)
...
include.rownames = FALSE, allign= "rllllllll", hline.after=NULL,
sanitize.text.function = identity,
add.to.row=list(pos=list(-1,-1, 0, 3), command=c(
"\\toprule", 
c("\\multicolumn{1}{l}{Noise} & \\multicolumn{4}{l}{Accuracy}  & \\multicolumn{4}{l}{Terminal nodes}\\\\",
"\\midrule",
"\\bottomrule"
)))
)
...
\begin{figure}[t!]
\centering
<<param-operator-plot, fig = TRUE, echo = FALSE, width=10, height=15.5>>=
...
\begin{figure}[t!]
\centering
<<param-ntrees-plot, fig = TRUE, echo = FALSE, width=10, height=5.2>>=
