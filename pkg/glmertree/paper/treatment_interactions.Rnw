...
The \verb|R| code for running the illustrative examples is provided in the Appendix.
...
\end{equation}
%
To estimate the parameters of this model, we take an approach similar to that of the mixed-effects regression trees (MERT) approach of \citeA{HajjyBell11} and \citeA{SelaySimo12}, but extend their ideas from classical CART trees with only random intercepts to a full GLMM algorithm. In the MERT approach, the fixed-effects part of a GLMM is replaced by a CART regression tree, and the random-effects part is estimated as usual. To estimate a MERT, an iterative approach is taken, alternating between (1) assuming random effects known, allowing for estimation of the regression tree, and (2) assuming the regression tree known, allowing for estimation of the random effects. 
