...
exampleglmmtree <- lmertree(outcome ~ treatment | age + duration + anxiety, outcome ~ (1|cluster), data=example)
@
...
Alternatively, if the recursive subgroup structure (i.e., the partition) were known, the tree could be estimated as a single GLM. The model could then be written: $g(\mu_{i}) = x_{i}^{* \top}\beta^{*}$, where $x_{i}^{*}$ are the values of the $2J$ interactions between the subgroups from the tree, and the elements of $x_{i}$. $\beta^{*}$ would also have length $2J$, and contain the subgroup-specific fixed-effects coefficients.
...
\vspace{5pt}
\noindent Step 0: Initialize by setting $r$ and all values $\hat{b}_{(r)}$ to 0.

\vspace{5pt}
\noindent \hangindent=4em Step 1: Set $r = r+1$. Estimate GLM tree ($x_{i}^{\top}\hat{\beta}_{j(r)}$), with $z_{i}^{\top}\hat{b}_{(r-1)}$ as an offset.

\vspace{5pt}
\noindent Step 2: Estimate random effects $z_{i}^{\top}\hat{b}_{(r)}$, with $x_{i}^{\top}\hat{\beta}_{j(r)}$ as an offset.
...
\verb|R| \cite{R14} was used for generation and analysis of all datasets. Two additional R packages were used: \verb|partykit| (version 1.0-2; \citeNP{HothyZeil15}) for the estimation of GLM trees, and \verb|lme4| (version 1.1-7; \citeNP{BateyMeac12}) for the estimation of random-effects coefficients. 
...
We build all trees using data of patients with complete observations; that is, observations with non-missing values for potential partitioning variables, and pre- and posttreatment HAM-D score. As a result, data from 694 patients from 7 studies were included for the analyses. Results of our analysis may therefore not be representative of the complete dataset of the meta-analysis by \cite{CuijyWeit14}. 
...
<<depression-trees>>=
formula <- HRSDt1 ~ Tx_group | Age + Gender + education + ComorbidAnxietyDisorder + HRSDt0

## GLM tree
lmtree_app <- lmtree(formula, data = metadata,
 		offset = fitted(lm(HRSDt1 ~ HRSDt0), data = metadata))

## GLMM tree
lmertree_app <- lmertree(formula, HRSDt1 ~ (1 | studyid) + HRSDt0, data = metadata,
 		initialRandomEffects = fitted(lm(HRSDt1 ~ HRSDt0, data = metadata)))
@
