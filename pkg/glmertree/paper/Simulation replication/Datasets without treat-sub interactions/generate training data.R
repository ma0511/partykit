...
corUbi[[2]] <- "bi and random U correlated"
...
for (d in 1:length(bi$sigmas)) {# d is counter for sigma of b_i's
  for (e in 1:length(bi$numbclus)) {# e is counter for number of clusters
    ...
    for (f in 1:length(corUbi)) { # f is counter for correlation between U and bi's
        ...
        for (h in 1:length(pXj)) { # h is counter for number of covariates
          ...
          for (i in 1:length(N)) { # i is counter for sample size N
            ...
            for (j in 1:length(rho)) { # j is counter for intercorrelations between covariates
              ...
              # generate initial Y (overall population mean)
              r$Y <- 30
                            
              # adjust initial Y for overall treatment effect
              ...
              if (corUbi[[f]]=="bi and random U correlated") {
