...
\item \pkg{RWeka} (J48, M5$^\prime$, LMT)
...
offers a unified representation of trees with methods for
\begin{itemize}
\item printing,
\item plotting,
\item computing predictions and
\item coercing from/to different formats.
\end{itemize}
...
My students implemented CHAID as a software project last winter. I wanted
them to focus on the algorithm, not on technical details. 

We only had to implement variable selection, splitting and stopping rules,
...
Benchmark experiment CHAID vs. CTREE\\
...
nd <- data.frame(nutzung = gl(3, 10), beruf = sample(gl(3, 10)))
