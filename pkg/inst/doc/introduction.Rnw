...
\author{Torsten Hothorn\\Ludwig-Maximilians-Universit\"at \\ M\"unchen
   \And Achim Zeileis\\Leopold-Franzens-Universit\"at \\ Innsbruck}
...
\Address{
  ...
  Achim Zeileis\\
  Institut f\"ur Statistik \\
  Fakult\"at f\"ur Volkswirtschaft und Statistik \\
  Leopold-Franzens-Universit\"at Innsbruck \\
  Universit\"atsstra{\ss}e 15 \\
  6020 Innsbruck, \"Osterreich \\
...
%%% Aufbau: erstmal Baum zusammenbauen, den plotten (ohne code) und diesen
%%% Baum dann Schritt fuer Schritt zusammensetzen vielleicht:

<<weather-data, echo = FALSE>>=
weather <-
structure(list(outlook = structure(c(1L, 1L, 2L, 3L, 3L, 3L, 
2L, 1L, 1L, 3L, 1L, 2L, 2L, 3L), .Label = c("sunny", "overcast", 
"rainy"), class = "factor"), temperature = c(85, 80, 83, 70, 
68, 65, 64, 72, 69, 75, 75, 72, 81, 71), humidity = c(85, 90, 
86, 96, 80, 70, 65, 95, 70, 80, 70, 90, 75, 91), windy = c(FALSE, 
TRUE, FALSE, FALSE, FALSE, TRUE, TRUE, FALSE, FALSE, FALSE, TRUE, 
TRUE, FALSE, TRUE), play = structure(c(2L, 2L, 1L, 1L, 1L, 2L, 
1L, 2L, 1L, 1L, 1L, 1L, 1L, 2L), .Label = c("yes", "no"), class = "factor")), .Names = c("outlook", 
"temperature", "humidity", "windy", "play"), row.names = c(NA, 
-14L), class = "data.frame")
@
%%% response: play siehe Witten/Frank im Weka Buch
...
($(\mu_1, \dots, \mu_{l - 1})$ = \code{break}). We are interested to split according to the 
...
t1 <- party(n1, 
   data = iris,
   fitted = data.frame("(fitted)" = fitted_node(n1, data = iris),
                       "(response)" = iris$Species,
                       check.names = FALSE))
t1
@

\section{My first Tree}
...
to set-up the data and call \code{growtree()}:
<<mytree-3, echo = TRUE>>=
...
\code{print()} method provides us with a first overview on the 
resulting model
<<mytree-4, echo = TRUE>>=
...
We depict the model graphically using \code{plot()} (see Figure~\ref{plottree})
...
aqt4
@
which again are objects inheriting from \code{party} and thus
...
<<mytree-10, echo = TRUE>>=
fun <- function(x) 
    format.pval(info_split(split_node(x))$pvalue, 
                digits = 3, eps = 0.001)
nid <- nodeids(aqt)
