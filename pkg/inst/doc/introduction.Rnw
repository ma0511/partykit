...
\author{Torsten Hothorn\\Ludwig-Maximilians-Universit\"at M\"unchen
   \And Achim Zeileis\\Leopold-Franzens-Universität Innsbruck}
...
\begin{eqnarray*}
x \mapsto g(x) = \sum_{j = 1}^l j I_{A(j)}(x_i)
\end{eqnarray*}
where $A(j) = (\mu_{j - 1}, \mu_j]$ for \code{right = TRUE} except $A(l) = (\mu_{l - 1}, \infty)$.
If \code{right = FALSE}, then $A(j) = [\mu_{j - 1}, \mu_j)$ except 
$A(1) = (-\infty, \mu_1)$. Note that with some categorical variable $x_i$ and default
...
to daugther node $d_1$ and observations with $x_i > \mu_1$ to $d_2$.
