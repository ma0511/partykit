...
in various software systems emerged: Typically the original authors of the
algorithms also provide software and furthermore collections of various
types of trees are available in many software systems such as
\pkg{Weka} \citep{Witten+Frank:2005} or the \proglang{R} system for
statistical computing \citep{R}. Within \proglang{R} the list of prominent packages includes
...
splits in the ``weather tree'' from Figure~\ref{fig:weather-plot}. The
...
reimplemented for every new kind of decision tree but simply be
...
@
%
Similar to the functions applied in a \fct{nodeapply}, the \fct{print} and
\fct{plot} methods can be customized through panel function that format
certain parts of the tree (such as header, footer, node, etc.). For example,
an extended formatting with multiple lines is easily accomodated by supplying
a character vector in every node:
%
...
An example for the information \emph{foo} might be \code{kidids}. Hence, in the printing example above
using \code{info_node(n)} rather than \code{n$info} for some node \code{n} would have been the
...
Each node has a unique identifier \code{id}. The root node has identifier 1;
the first daughter node has identifier 2, those daughter (if present) has identifier
3 and so on. So, we first look at the root node, visit the first daugther node and all
its daugthers, then the second daughter node and all its daughters and so on. This is
called \emph{pre-order tree traversal}, see \cite{tree_traversal}. A node consisting only
...
\setkeys{Gin}{width=\textwidth}
%
This has an enhanced \fct{plot} and \fct{predict} method. For example, \code{plot(t2)}
now produces stacked bar plots in the leaves (see Figure~\ref{fig:constparty-plot})
as \code{t2} is a classification tree. For regression and survival trees, boxplots and
Kaplan-Meier curves are employed automatically, respectively.
