...
In the more than fifty years since the seminal paper
of \cite{Morgan+Sonquist:1963} on ``automatic interaction detection''
appeared, a wide range of methods have been suggested that are often
...
CTree \citep{Hothorn+Hornik+Zeileis:2006}, among many others. See
...
\pkg{RWeka} \citep[containing interfaces to J4.8, M5', LMT]{RWeka},
...
very similar challenges. However, due to the fragmentation of the different
communities -- ranging from statistics over machine learning to various
applied fields -- many discussions of algorithms do not reuse established
theoretical results and terminology. Similarly, there is no common ``language''
for the software implementations and different solutions are provided by
different packages (even within \proglang{R}) with relatively little
reuse of code.

The \pkg{partykit} tries to improve the computational situation by
providing a common unified infrastructure for recursive partytioning, especially
representing fitted trees along with printing, plotting, and
computing predictions. The design principles are:
\begin{itemize}
  \item One `agnostic' base class (`\code{party}') which can encompass an extremely wide
        range of different types of trees.
  \item Subclasses for important types of trees, e.g., trees with constant
        fits (`\code{constparty}') or with parametric models (`\code{modelparty}')
...
Explain purpose of partykit:

- basic infrastructure toolkit (in this document)

- infrastructure for constant-fit trees (briefly shown here
but more in constparty.Rnw) along with coercion functions

- reimplementation of functions from party: ctree and mob.
(The latter also provides some general tools for modeparty objects.)
...
a simple artificial data set from the \cite{Witten+Frank:2005} book
