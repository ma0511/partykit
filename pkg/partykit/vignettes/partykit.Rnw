...
using \code{info_node(n)} rather than \code{n$info} would have been the
...
\fixme{In the predictions below the outlook variable needs to be a factor. If only a
character variable is supplied, its levels are not ordered correctly. I would have
expected that this is handled correctly by the data preprocessing.}
