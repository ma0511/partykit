...
the parameter instability test results.
...
<<>>=
bh_tree <- lmtree(medv ~ lstat + rm | zn + indus + chas + nox + age +
  dis + rad + tax + crim + b + ptratio, data = BostonHousing)
@

The result is the fitted model \code{bh_tree} of class ``\code{mob}'' that contains
...
<<echo=FALSE>>=
nt <- NROW(coef(bh_tree))
nk <- NCOL(coef(bh_tree))
@
