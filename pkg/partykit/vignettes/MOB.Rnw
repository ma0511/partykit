...
built on top of `\code{party}' objects,
special infrastructure for fitted models,
`mobsters' such as \code{lmtree()}, \code{glmtree()}
...
are displayed both in the print output and the visualization (see Section~\ref{fig:pid_tree}
