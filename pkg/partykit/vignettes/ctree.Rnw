...
\Abstract{
  The \pkg{partykit} package \citep{Hothorn+Hornik+Zeileis:2006} aims
...
information measure.
\end{quote}
With the \pkg{partykit} package \citep[see][for a full description of its
...
In package \pkg{partykit}, the dependency structure and the variables may be
...
The \code{predict} function aims at computing predictions in the space of
...
first reported by \cite{Aart+Smeenk:1975} and 
...
replications. The prognostic factors \code{twigs} and \code{water}
found by \cite{Death:2002} are confirmed by the model 
