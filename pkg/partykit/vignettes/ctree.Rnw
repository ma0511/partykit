...
This vignette describes \code{partykit::ctree}, an implementation of
conditional inference trees \citep{Hothorn+Hornik+Zeileis:2006}.  The
reference implementation of this method is \code{party::ctree}.  Both
functions produce the same output, however, transformations of the
explanatory variables, resampling distributions of the test statistics, and
extraction of split-statistics are not yet implemented in \code{party::ctree}.

\fixme{Communicate partykit to be the reference implementation now -- except
for few features that have not yet been ported.}
