...
\Abstract{
  This vignette describes an implementation 
  of conditional inference trees which
...
conditional inference trees \citep{Hothorn+Hornik+Zeileis:2006}.  The original
implementation of this method is \code{party::ctree} and the 
reference implementation is \code{partykit::ctree}.  Both
functions produce essentially the same output, with the following exceptions: 
\begin{itemize}
\item transformations of the explanatory variables,
\item resampling distributions of the test statistics, and
\item extraction of split-statistics are not yet implemented in \code{partykit::ctree}.
\end{itemize}
There is a new argument \code{majority = FALSE} in \code{partykit::ctree\_control}
...
(as also implemented in \code{partykit::ctree}). Both implementations do not
fit surrogate splits by default, so users need to make sure to set the \code{maxsurrogate}
argument if missing values are a problem in their learning or testing data.
\code{party::ctree} did an exhaustive search for finding surrogate splits in
...
The breaking of ties between variables with almost identical selection criterion (for example,
a $p$-value being almost zero) is done with respect to the test-statistics in \code{partykit::ctree},
so in very rare cases there might be different split variables in some nodes. The implementation of 
\code{ctree} in package \pkg{partykit} is almost entirely written in \proglang{R}, whereas 
the original implementation in \pkg{party} is almost entirely written in \proglang{C}.
...
\setkeys{Gin}{height=.8\textheight, keepaspectratio=TRUE}
\begin{figure}[t]
\begin{center}
<<spider-plot, echo = TRUE, fig = TRUE, width = 10, height = 20>>=
plot(sptree)
@
