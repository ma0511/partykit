...
PMML (Predictive Model Markup Language) that does not provide the full learning
...
@
The object \code{rp} of class \class{rpart} can be coerced to
a \class{constparty} by the \fct{as.party} function. Internally, this transforms
...
package \pkg{pmml}, part of the \pkg{rattle} project, that allows to
...
findsplit <- function(response, data, weights, alpha = 0.01) {
  ...
  if(length(lev) == 2) {
    ...
    xlogp <- min(sapply(comb, function(q) mychisqtest(x %in% q)))
    splitpoint <- comb[[which.min(xlogp)]]
  }

  ## split into two groups (setting groups that do not occur to NA)
  splitindex <- levels(data[[xselect]]) %in% splitpoint + 1L
...
which nodes to prune, Here, one idea would be to stop splitting when
the $p$-value is larger than $10^{-5}$ rather than at $10^{-2}$ as
used by default in \fct{mytree}. Hence we use \fct{nodeapply} to
...
\code{type = "quantile"} is available. \fixme{Is this correct?}
...
\fixme{illustrate flexible FUN interface as well}

\section{Conclusion} \label{sec:conclusion}

The classes \class{constparty} and \class{simpleparty} can be used to represent
trees with constant fits in the terminal nodes, including most of the traditional tree variants. 
For a number of implementations it is possible to convert the resulting
trees to one of these classes,  thus offering unified
methods for handling constant-fit trees. Also, computing non-standard predictions, such as the
...
algorithm using the \pkg{partykit} infrastructure.

\fixme{maybe improve conclusions some more}
