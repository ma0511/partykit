...
algorithm (in fact so simple that we would advise not to use it for any real
...
findsplit <- function(response, data, weights, alpha = 0.01) {
  ...
  mychisqtest <- function(x) {
    x <- factor(x)
    if(length(levels(x)) < 2) return(0)
    ct <- suppressWarnings(chisq.test(table(y, factor(x)), correct = FALSE))
  ...
  ## split into two groups (setting groups that don't occur to NA)
  ...
  return(partysplit(varid = as.integer(xselect),
    index = splitindex, info = list(p.value = exp(logp))))
}
@
...
growtree <- function(id = 1L, response, data, weights) {
  ...
  return(partynode(id = as.integer(id), split = sp, kids = kids,
    info = list(p.value = min(info_split(sp)$p.value))))
}
@
...
appropriate for a certain response. The \code{print}, \code{plot}, and \code{predict}
...
\begin{figure}[t!]
\centering
<<mytree-8, echo = FALSE, fig = TRUE, width = 10, height = 6>>=
plot(myttnc9)
@
...
to prune nodes based on their node id or their names
using \fct{nodeprune}, e.g., pruning all children of nodes 4, 11, and 14
(see Figure~\ref{prunetree}).
<<mytree-5-nodeprune, echo = TRUE, eval=FALSE>>=
plot(nodeprune(myttnc, c(4, 11, 14)))
@

\begin{figure}[t!]
\centering
<<mytree-5-nodeprune-plot, echo = FALSE, fig = TRUE, height=8.5, width=14>>=
<<mytree-5-nodeprune>>
@
...
inner nodes:
<<mytree-10, echo = TRUE>>=
fun <- function(x) format.pval(info_node(x)$p.value,
  digits = 3, eps = 0.001)
nid <- nodeids(myttnc)
