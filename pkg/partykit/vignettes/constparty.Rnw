...
\Abstract{
We describe how one can coerce objects representing trees to objects represented by 
...
@
The \code{rp} object of class \code{\Sexpr{class(rp)[1L]}} can be coerced to an
...
@
\fixme{Z: now all the PMML examples work, however I would not use the online examples
in the vignette because one might want to run the vignette when offline. I think
the QUEST example would be sufficient. Additionally, one could show the RPart export
plus import.}
<<PMML-read>>=
...
Possibly this is more for an appendix than the main paper. It does:
bring the data into the same shape as it is assumed to be by \code{ttnc_quest}.
Then a new \code{party} is set up and coerced to \code{constparty}.
...
\begin{figure}
\centering
<<PMML-Titanic-plot1, echo=FALSE, fig=TRUE, height=9.5, width=16.5>>=
plot(ttnc_quest)
@

<<PMML-Titanic-plot2, echo=FALSE, fig=TRUE, height=8.5, width=14>>=
plot(ttnc_cp)
@
\end{figure}
...
table(predict(ttnc_quest, newdata = ttnc), predict(myttnc, newdata = ttnc))
@
