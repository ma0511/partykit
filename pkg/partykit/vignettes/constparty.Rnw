...
\code{type = "quantile"} and \code{type = "density"} is available.
...
@
The \fct{predict} method features a \code{FUN} argument that can be used to
...
predict(myttnc, newdata = nttnc, FUN = function(y, w)
    rank(prop.table(table(rep(y, w)))))
@
The user-supplied function \code{FUN} takes two arguments, \code{y} is the
response and \code{w} is a vector of weights (case-weights in the case here). Of course,
...
for illustration generalizes to situations where this is not possible as well.
...
framework.  It is rather straightforward to implement a new (or old) tree
algorithm using the \pkg{partykit} infrastructure.  Thus, a prototype
implementing a fancy idea on how to improve trees is only a couple lines of
\textsf{R} code away.
