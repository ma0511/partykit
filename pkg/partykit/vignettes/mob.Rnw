...
presents further illustrative examples \citep[including replications from][]{Zeileis+Hothorn+Hornik:2008}
...
\item \emph{Parameter instability tests:} To assess the stability of all model parameters
  ...
  \cite{Merkle+Fan+Zeileis:2013} have been added as further options (but are not used
...
In this section, further empirical illustrations of the MOB method are
...
\begin{enumerate}
  ...
  \item Teaching ratings.
  \item Titanic survival.
...
<<Titanic-tree>>=
ttnc_tree <- glmtree(Survived ~ Treatment | Class + Gender + Age, data = ttnc,
...
\begin{figure}[t!]
\setkeys{Gin}{width=\textwidth}
<<Titanic-plot,echo=FALSE,results=hide,fig=TRUE,height=6,width=9>>=
plot(ttnc_tree)
@
