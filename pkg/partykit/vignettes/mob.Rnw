...
%\VignetteDepends{AER,Formula,mlbench,sandwich,strucchange,survival,TH.data,vcd}
...
this supported many applications of interest, it was somewhat limited for
certain extensions: (1)~The \proglang{S}4 wrappers for the models were
...
is provided in \pkg{partykit} \citep{partykit}: (1)~New models are much easier
...
The remainder of this manuscript is organized as follows: Section~\ref{sec:algorithm}
...
presents replications of further illustrative examples from \cite{Zeileis+Hothorn+Hornik:2008}
...
\item \emph{Parameter instability tests:} To assess the stability of all model parameters
  ...
  \cite{Merkle+Fan+Zeileis:2013} have been added as further options.
\item \emph{Partitioning:} As the objective function $\Psi$ is additive, it is easy
  ...
  the latter is not implemented in the software; but optionally categorical partitioning
...
options can be set up by the \code{mob_control()} function. Additional arguments \code{...}
...
\verb:y ~ x1 + ... + xk | z1 + ... + zl: 
\fixme{Add note: first part of rhs can include interactions, transformations, etc (?)}
where the variables on the
...
\begin{itemize}
  ...
  \item \code{objfun}: Value of the minimized objective function $\sum_{i = 1}^n \Psi(y_i, x_, \hat \theta)$.
...
function but avoiding further computations.
...
\code{fit} needs to returned a classed model object for which methods to
\code{coef()}, \code{logLik()}, and \code{estfun()} \citep[see][]{sandwich}
...
<<PimaIndiansDiabetes-plot2,echo=FALSE,results=hide,fig=TRUE,height=6,width=9>>=
plot(pid_tree2)
@
...
category of each possible partitioning variable. See also \cite{Merkle+Fan+Zeileis:2013} for
...
also consistent any parameter instabilities across ordered variables, it does
not exploit the ordering information.

Recently, \cite{Merkle+Fan+Zeileis:2013}
proposed an adapted max\textit{LM} test for ordered variables along with a
...
the corrsponding information matrix can be used if: (a)~the estimating functions
...
option in \code{mob_control()} is to set \code{vcov = "info"} rather than
\code{vcov = "opg"} (the default).
...
\fixme{Do we need sctest(,log = TRUE)?}
%
...
this means that typically a lot of subset models have to be fit and often this will not
...
setting \code{splits = FALSE} suppresses counting the splits in the degrees of freedom),
...
\fixme{type = "node": OK; type = "response" has a different meaning as predict.glm;
       type = "prob" is equal to type = "response" in predict.glm}
...
In this section, three further empirical illustrations of the MOB method are
provided, replicating the results from \cite{Zeileis+Hothorn+Hornik:2008}:
\begin{enumerate}
  ...
  \item Modeling of breast cancer survival and by capturing heterogeneity
    in certain (treatment) effects across individuals.
\end{enumerate}
More details about the underlying data sets, previous studies exploring
...
and explore the resulting trees. The first two illustrations employ the
\code{lmtree()} convenience function while the latter uses \code{survreg()}
...
j_tree <- lmtree(log(subs) ~ log(price/citations) | price + citations +
  age + chars + society, data = Journals, minsplit = 10, verbose = TRUE)
@
%
...
%
<<gbsg2_tree>>=
gbsg2_tree <- mob(Surv(time, cens) ~ horTh + pnodes | age + tsize + tgrade +
  progrec + estrec + menostat, data = GBSG2,
  fit = wbreg, control = mob_control(minsplit = 80))
@
%
...
To allow gain some efficiency in the computations and to allow for further customization
