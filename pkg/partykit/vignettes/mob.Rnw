...
\citep[][employ this idea in psychometric item response theory models]{Strobl+Kopf+Zeileis:2013}.
...
is: \code{(2 * objfun[1] + log(nobs[2]) * df[1]) < (2 * objfun[2] + nobs[2] * df[2])}. As the
...
(\code{Topmodel2007} from \pkg{psychotree}), a parametric model for paired
comparison data (\code{btReg.fit()} from \pkg{psychotools}, implementing the
Bradley-Terry model), and an extractor method for the corresponding empirical
estimating functions (which are computed by default in \code{btReg.fit()} already
and just need to be extracted):
<<bt-packages>>=
...
log-linear GLM. This is the approach used internally by \code{btReg.fit()}.
...
@
%
The function \code{btfit1()} simply calls \code{btReg.fit()} ignoring all arguments
except \code{y} as the others are not needed here. No more processing is required
because \class{btReg} objects come with a \code{coef()} and \code{logLik()} method
and an \code{estfun()} method was already defined above. Hence, we can call \code{mob()}
...
@
%
Still \code{btReg.fit()} is employed for fitting the model but the quantities
\code{estfun} and \code{vcov} are only computed if they are really required. This
saves some computation time (about 20\% on the authors' machine) when computing
the tree:
%
<<bt2>>=
...
data. The default plot (upper panel) reports the estimated coefficients on the log scale and
...
\class{btReg} objects from \pkg{partykit} and \code{nodeapply} this to
