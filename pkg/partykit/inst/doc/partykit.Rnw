...
\Abstract{
  ...
  \code{print}/\code{plot}/\code{predict} methods.
}
...
the function \fct{party} takes several further arguments including
\code{fitted} values, a \code{terms} object, arbitrary additional
\code{info}, and some further arguments (\code{...}):
\begin{Code}
  party(node, data, fitted = NULL, terms = NULL, ..., info = NULL)
\end{Code}

The tree \code{py} can then be easily visualized by
<<weahter-plot, eval=FALSE>>=
plot(py)
@
yielding Figure~\ref{weather-plot}.
...
%kidids_node(partynode_object)
%kidids_split(partynode_object)
