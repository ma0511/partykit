...
a new flexible class \class{party} comprising essentially a recursive list of \class{partynode}
objects along with \class{partysplit} information. Additionally, further
information about the underlying data as well as the associated model fit
can be stored and accessed. Therefore, the \class{party} class can capture
...
under development at \proglang{R}-Forge but already provides conversion methods for
...
In our presentation, we discuss the design of the new \class{party} class
and its methods and illustrate how it can be utilized to interface other classes such
as \class{rpart} or \class{J48}, obtain new implementations of existing algorithms
such as CHAID or re-implement techniques such as model-based recursive partitioning
\citep[MOB,][]{partykit:Zeileis+Hothorn+Hornik:2008}.
