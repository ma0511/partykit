...
objective function $\Psi(y, x, \theta)$ for $n$ observations $y_i$ ($i = 1, \dots, n$):
...
\begin{frame}[fragile]
\frametitle{Implementation: Class}

\textbf{Class:} `\code{modelparty}'.
...
\textbf{Model:} Paired comparison via Bradley-Terry.
\begin{itemize}
...
\textbf{Here:} Use \code{mob()} directly to build model from scratch.
