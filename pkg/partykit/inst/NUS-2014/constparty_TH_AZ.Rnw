...
<<rpart, eval=FALSE>>=
rp <- rpart(Survived ~ Gender + Age + Class, 
            data = ttnc)
@
...
<<pmml, eval=FALSE>>=
pm <- pmmlTreeModel(file.path(system.file("pmml", 
    package = "partykit"), "ttnc.pmml"))
@
...
<<mob>>
<<mytree>>
save(rp, ct, ev, pm, mb, myttnc, file = "slides-trees.rda")
}
@

<<J48>>=
j48 <- J48(Survived ~ Gender + Age + Class, 
           data = ttnc)
@
...
system.time(p1 <- predict(rp, newdata = nd, 
                          type = "class"))
...
\setkeys{Gin}{width=1.1\textwidth}
\hspace*{-0.4cm}%
<<Titanic-pmml-visualization, fig=TRUE, height=10, width=16>>=
plot(pm)
@
...
plot(mytree(Survived ~ Class + Age + Gender, 
            data = ttnc))
@
...
as explained in the \code{constparty} package vignette.
...
Hothorn T, Zeileis A (2014).
 ...
 R~package vignette version~0.2-0.
