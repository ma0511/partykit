...
\title{\pkg{partykit}: An Open-Source Toolkit for Recursive Partytioning in \proglang{R}}
\editor{(submission for MLOSS track)}
...
\proglang{R}-Forge \citep{Theussl+Zeileis:2009}

The vignette (\code{"partykit"}) introduces the basic \class{party} class
...
\class{rpart} tree visualized by \pkg{partykit} (left) and
