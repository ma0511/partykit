...
predictions (e.g., a mean or proportion) rather than all observations of the
response that was used for tree learning.  So far, the PMML interface has
...
or represent trees learned in other systems. Here, we briefly outline
