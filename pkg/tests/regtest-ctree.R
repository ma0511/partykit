...
fun <- function(args) {
    ...
    old <- oldmod@get_where(newdata = test)
    ...
    if (inherits(newmod, "try-error")) {
        new <- rep(1, length(old))
    } else {
        new <- predict(newmod, type = "node", newdata = test)
    }
    cc <- complete.cases(test)
    list(time = c(ot, nt), tab = table(old[cc], new[cc]), 
         tab2 = table(old, new), learn = learn, test = test)
}
...
err <- sapply(ret, function(x) { all(rowSums(x$tab > 0) == 1) & all(colSums(x$tab > 0) == 1) })

save(ret, tm, err, tgr, fun, gr, dgp, file = "results-regtest.Rda")
