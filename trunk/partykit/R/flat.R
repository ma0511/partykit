flat2rec <- function(obj) {
    ...
    new_recnode <- function(id) {
        if (is.null(kids_node(obj[[id]])))
            ...
            partynode(id = id, split = split_node(obj[[id]]),
                 kids = lapply(kids_node(obj[[id]]), new_recnode),
		 surrogates = surrogates_node(obj[[id]]),
                 info = obj[[id]]$info)
    }
        
    node <- partynode(id = as.integer(1), split = split_node(obj[[1]]),
...
rec2flat <- function(node) {
    ...
    new_flatnode <- function(node) {
        if (is.terminal(node))
            obj[[node$id]] <<- partynode(id = id_node(node), info = node$info)
        else {
            obj[[node$id]] <<- partynode(id = id_node(node), split = split_node(node),
